...
const Home: NextPage = () => {
  ...
  async function generatePhoto(fileUrl: string) {
    ...
    setLoading(true);
    const res = await fetch("/api/remove-scratch", {
      method: "POST",
      headers: {
  ...
  return (
    <div className="flex max-w-6xl mx-auto flex-col items-center justify-center py-2 min-h-screen">
      ...
      <main className="flex flex-1 w-full flex-col items-center justify-center text-center px-4 mt-4 sm:mt- sm:mb-0 mb-8">
        {/* <WatchTutorial /> */}
        <h1 className="mx-auto max-w-xs sm:max-w-5xl font-display text-4xl font-bold tracking-normal text-slate-900 sm:text-6xl my-5">
          Loại bỏ{" "}
